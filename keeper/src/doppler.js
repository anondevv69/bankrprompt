...
export async function claimDopplerIfAvailable(publicClient, wallet, router, projectToken) {
  const beneficiary = router;
  const status = await fetchClaimable(beneficiary, projectToken);
  if (!status.eligible) {
    console.log("doppler not eligible for router", projectToken);
    return;
  }
