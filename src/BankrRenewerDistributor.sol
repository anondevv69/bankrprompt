...
/// @title BankrRenewerDistributor
/// @notice Push-distributes project-token fees to Bankr Club renewers per epoch on Base.
contract BankrRenewerDistributor {
    enum Phase {
        Open,
        Locked
    }
    ...
    function openRound(address token) external onlyKeeper returns (uint256 roundId) {
        if (token == address(0)) revert ZeroAmount();
        roundId = roundCount++;
        Round storage r = _rounds[roundId];
        r.token = token;
        r.phase = Phase.Open;
        emit RoundOpened(roundId, token);
    }

    /// @notice Pull tokens from the caller into an open round.
    function fundRound(uint256 roundId, uint256 tokenAmount) external {
        Round storage r = _rounds[roundId];
        if (r.phase != Phase.Open) revert WrongPhase();
        if (tokenAmount == 0) revert ZeroAmount();
        if (!IERC20(r.token).transferFrom(msg.sender, address(this), tokenAmount)) revert TransferFailed();
        r.payoutAmount += tokenAmount;
        emit RoundFunded(roundId, tokenAmount, r.payoutAmount);
    }

    /// @notice Credit tokens already sitting on this contract (e.g. routed from BankrFeeRouter).
    ...
    function sweepDust(uint256 roundId, address to) external onlyOwner {
        ...
        r.paidOut += dust;
        if (!IERC20(r.token).transfer(to, dust)) revert TransferFailed();
        emit DustSwept(roundId, r.token, to, dust);
    }
